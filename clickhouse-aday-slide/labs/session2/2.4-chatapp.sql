...
(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Please review the invoice for Project Alpha' AND _sign = 1 LIMIT 1),
 'invoice', '/files/invoices/inv_001.pdf', 256000, 'application/pdf',
 '2024-01-15 10:30:00', 1500.00, 'USD', 'pending', 1), 

(generateUUIDv4(),
 (SELECT message_id FROM messages WHERE content = 'Here is the receipt for office supplies' AND _sign = 1 LIMIT 1),
 'receipt', '/files/receipts/rec_001.pdf', 128000, 'application/pdf',
 '2024-01-16 14:20:00', 299.99, 'USD', 'paid', 1), 

(generateUUIDv4(),
 (SELECT message_id FROM messages WHERE content = 'Check out our new product catalog' AND _sign = 1 LIMIT 1),
 'other', '/files/documents/catalog_2024.pdf', 1024000, 'application/pdf',
 '2024-01-17 15:00:00', 0.00, 'USD', 'pending', 1); 

-- Example of updating a message (using CollapsingMergeTree)
-- First, mark the old message as deleted
INSERT INTO messages VALUES
(generateUUIDv4(), 1001, 101, 'text', 'Hello team!', 0, '2024-01-15 09:15:00', -1);
...
-- Example of updating payment status (using CollapsingMergeTree)
-- First, mark the old status as deleted
...
(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Q1 consulting invoice attached' AND _sign = 1 LIMIT 1),
 'invoice', '/files/invoices/consulting_q1.pdf', 345000, 'application/pdf',
 '2024-01-18 16:30:00', 5000.00, 'USD', 'pending', 1),

(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Project timeline updated' AND _sign = 1 LIMIT 1),
 'other', '/files/documents/timeline_v2.xlsx', 250000, 'application/xlsx',
 '2024-01-19 09:15:00', 0.00, 'USD', 'pending', 1),

(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Software license renewal invoice' AND _sign = 1 LIMIT 1),
 'invoice', '/files/invoices/license_2024.pdf', 280000, 'application/pdf',
 '2024-01-19 11:20:00', 2499.99, 'USD', 'pending', 1),


(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Client meeting expenses receipt' AND _sign = 1 LIMIT 1),
 'receipt', '/files/receipts/client_meeting.pdf', 156000, 'application/pdf',
 '2024-01-18 10:00:00', 245.50, 'USD', 'paid', 1),

(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Updated sales presentation' AND _sign = 1 LIMIT 1),
 'other', '/files/presentations/sales_q1.pptx', 1500000, 'application/pptx',
 '2024-01-18 13:45:00', 0.00, 'USD', 'pending', 1),

(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Marketing campaign invoice' AND _sign = 1 LIMIT 1),
 'invoice', '/files/invoices/marketing_jan24.pdf', 425000, 'application/pdf',
 '2024-01-19 10:00:00', 7500.00, 'USD', 'pending', 1),

(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Trade show booth payment receipt' AND _sign = 1 LIMIT 1),
 'receipt', '/files/receipts/tradeshow_booth.pdf', 198000, 'application/pdf',
 '2024-01-19 16:45:00', 3500.00, 'USD', 'paid', 1),


(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Office supplies invoice' AND _sign = 1 LIMIT 1),
 'invoice', '/files/invoices/office_supplies_jan.pdf', 175000, 'application/pdf',
 '2024-01-18 14:20:00', 458.75, 'EUR', 'paid', 1),

(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Company handbook updated' AND _sign = 1 LIMIT 1),
 'other', '/files/documents/handbook_2024.pdf', 2500000, 'application/pdf',
 '2024-01-19 09:30:00', 0.00, 'USD', 'pending', 1),

(generateUUIDv4(), 
 (SELECT message_id FROM messages WHERE content = 'Catering service invoice for team lunch' AND _sign = 1 LIMIT 1),
 'invoice', '/files/invoices/catering_jan24.pdf', 165000, 'application/pdf',
 '2024-01-19 13:15:00', 850.00, 'EUR', 'pending', 1);